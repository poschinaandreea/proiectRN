...
**Student:** Poșchină Liliana Andreea <br>
**Link Repository GitHub** https://github.com/poschinaandreea/proiectRN.git <br>
**Data:** [Data]  
---
...
|------------------------------------------------------------------|--------------------------------|--------------------------------|
| Monitorizarea spațiilor de lucru pentru anomalii de comportament | Clasificarea secvențelor video bazate pe durată imobilitate și scor mișcare → alertă operator în < 100ms | RN + Web Service / UI |
| Identificarea staționării neautorizate în zone de securitate     | Extracție de caracteristici (Durată/Mișcare) pe fiecare cadru → predicție > 90% acuratețe pentru cazurile de imobilitate prelungită | Data Logging / Acquisition + RN |
| Analiza riscului post-eveniment                                                                 |Punerea la dispoziție a scorului de suspiciune și probabilității pentru fiecare cadru analizat | RN + Web Service / UI|
| 
**Instrucțiuni:**
...
**Total observații finale:** ~5000 (estimat dintr-un videoclip de ~30 secunde la 20 FPS)
**Observații originale:** ~5000 (100%)

**Tipul contribuției:**
[X] Date generate prin simulare fizică (Simularea mișcării/duratei este bazată pe ecuații)
[ ] Date achiziționate cu senzori proprii
[X] **Etichetare/adnotare manuală** (Eticheta finală 'label' este adăugată manual după extracția caracteristicilor)
[ ] Date sintetice prin metode avansate

**Descriere detaliată:**
Contribuția noastră originală este de **100%** și constă în **extracția și etichetarea manuală** a caracteristicilor din date video reale sau simulate.

1. **Extracție:** Scriptul `generate_video_dataset.py` folosește OpenCV (Haar Cascade) pentru a detecta fața subiectului și calculează două caracteristici esențiale pentru fiecare cadru:
    * **`durata_sec`:** Timpul cumulativ cât subiectul a rămas sub un prag de mișcare (imobilitate).
    * **`miscare`:** Un scor normalizat (0-1) bazat pe distanța în pixeli parcursă de centrul feței între cadre.
2. **Generare:** Acest proces transformă semnalul vizual brut într-un set de date tabelar (`video_dataset.csv`), înlocuind generarea matematică inițială.
3. **Etichetare Manuală:** Coloana finală **`label` (0/1)** este adăugată și completată manual pe baza momentelor din videoclip pe care le-am definit ca fiind suspicioase (imobilitate prelungită > 10 secunde).

Aceste date sunt direct relevante, deoarece caracteristicile extrase (`durata_sec` și `miscare`) sunt exact input-ul necesar modelului de clasificare.

**Locația codului:** `src/data_acquisition/generate_video_dataset.py`
**Locația datelor:** `data/raw/video_dataset.csv`
...
Am ales arhitectura de **Monitorizare Continuă cu Clasificare pe Secvență** (similară cu Monitorizarea Procesului Industrial) deoarece proiectul nostru necesită procesarea unui flux continuu (video) și luarea unei decizii binare (Suspect/Normal) bazate pe o acumulare de date în timp (durata imobilității).

Stările principale sunt:
1. **IDLE**: Sistemul așteaptă o cerere de analiză video sau declanșarea unui flux.
2. **ACQUIRE_FRAME**: Se citește un nou cadru din videoclip (sau de la cameră).
3. **FEATURE_EXTRACTION**: Se detectează fața/obiectul și se calculează caracteristicile `durata_sec` și `miscare`.
4. **RN_INFERENCE**: Caracteristicile sunt trimise către modelul RN (Regresie Logistică) pentru a obține predicția binară (0 sau 1).
5. **DISPLAY/OUTPUT**: Rezultatul (Suspect/Normal) este afișat (pe ecran/UI).
6. **LOG_RESULT**: Caracteristicile și predicția sunt înregistrate în baza de date sau un fișier log.

Tranzițiile critice sunt:
- **ACQUIRE_FRAME → LOG_RESULT**: Bucla principală care rulează continuu cât timp există cadre.
- **LOG_RESULT → ACQUIRE_FRAME**: Buclează pentru a trece la următorul cadru.
- **ACQUIRE_FRAME → STOP**: Când videoclipul se termină (`ret = False`) sau utilizatorul apasă tasta 'q'.
- **ACQUIRE_FRAME → ERROR**: Când `cap.isOpened()` este fals (videoclipul nu poate fi citit - EROARE FATALĂ).

Starea **ERROR** este esențială pentru că în mediul de lucru real, fluxul video poate fi întrerupt (cameră deconectată, fișier corupt). Trebuie să gestionăm aceste erori prin oprirea și notificarea utilizatorului.

Bucla de feedback **nu este prezentă** în sensul clasic (nu ajustăm parametrii camerei sau ai procesului), dar există o **buclă de inferență continuă** care oferă feedback vizual operatorului.
